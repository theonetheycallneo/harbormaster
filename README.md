...
- **Smoke test** — `npm run smoke` exercises the whole story against a running server: sign-up → API key → ingest → MCP initialize → auth rejection → `rag_search` relevance → fleet discovery → dispatch.
...
- Reranking stage + retrieval eval harness (golden-set recall@k before/after any chunking change)
